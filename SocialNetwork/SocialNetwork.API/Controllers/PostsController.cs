...
public class PostsController : ControllerBase
{
    ...
    [HttpPost]
    [Route("{postId}/likes")]
    public virtual async Task<ActionResult<PostLikeResponseDto>> PostPostsPostIdLikes([FromRoute, Required] uint postId)
    {
        var userId = HttpContext.GetAuthenticatedUserId();
        var postLike = await _postService.LikePost(userId, postId);        
        return Ok(postLike);
    }
    ...
    [HttpGet]
    [Route("{postId}/likes")]
    public virtual async Task<ActionResult<List<PostLikeResponseDto>>> GetPostsPostIdLikes(
        [FromRoute, Required] uint postId,
        [FromQuery, Required] int limit,
        [FromQuery] int currCursor)
    {
        var postLikes = await _postService.GetLikes(postId, limit, currCursor);        
        return Ok(postLikes);
    }
    ...
    [HttpDelete]
    [Route("{postId}/likes")]
    public virtual async Task<ActionResult<PostLikeResponseDto>> DeletePostsPostIdLikes([FromRoute, Required] uint postId)
    {
        var userId = HttpContext.GetAuthenticatedUserId();
        var postLike = await _postService.UnlikePost(userId, postId);
        return Ok(postLike);
    }
    ...
    public virtual async Task<ActionResult<List<CommentResponseDto>>> GetPostsPostIdComments(
        [FromRoute, Required] uint postId,
        [FromQuery, Required] int limit,
        [FromQuery] int currCursor)
    {        
        var comments = await _postService.GetComments(postId, limit, currCursor);
        return Ok(comments);        
    }
}